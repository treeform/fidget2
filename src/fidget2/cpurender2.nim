...
proc drawNodeInternal*(node: Node) =
  ...
  of nkRectangle, nkFrame, nkGroup, nkComponent, nkInstance:
    ...
    if false and node.strokes.len > 0:
  ...
  of nkVector, nkStar, nkEllipse, nkLine, nkRegularPolygon:
    if node.fills.len == 1 and node.fills[0].kind == pkSolid:
      var fillColor = node.fills[0].color.rgba.toPremultipliedAlpha()
      for geom in node.fillGeometry:
        screen.fillPath(
          geom.path,
          fillColor,
          mat * translate(node.pixelBox.xy),
          geom.windingRule
        )

    if node.strokes.len == 1 and node.strokes[0].kind == pkSolid:
      var strokesColor = node.strokes[0].color.rgba.toPremultipliedAlpha()
