...
proc main(r = "", e = "", l = 10000) =
  ...
  for frame in figmaFile.document.children[0].children:
    ...
    let image = drawCompleteGPUFrame(frame)

    let frameTime = epochTime() - startTime
    renderTime += frameTime
    image.writeFile("tests/frames/gpu/" & frame.name & ".png")
    ...
    if fileExists(&"tests/frames/masters/{frame.name}.png"):
      ...
      count += 1
    framesHtml.add(&"""<h4>{frame.name}</h4><p>{frameTime}s {diffScore} diffpx</p><img src="gpu/{frame.name}.png"><img src="masters/{frame.name}.png"><img src="diffs/{frame.name}.png"><br>""")

  framesHtml.add(&"<p>Total time: {renderTime}s</p>")
  writeFile("tests/frames/index.html", framesHtml)

  echo "see: tests/frames/index.html"
