...
proc downloadImages(fileKey: string, figmaFile: FigmaFile) =
  if not dirExists(rootPath / "images"):
  ...
  for kind, path in walkDir(rootPath / "images", relative = true):
    case kind:
    of pcFile:
    ...
    of pcDir:
      removeDir(rootPath / "images" / path)
    of pcLinkToFile, pcLinkToDir:
      removeFile(rootPath / "images" / path)
...
proc downloadFont(fontPostScriptName: string) =
  ...
  if not fileExists(fontPath) and not fileExists(fontUserPath):
    ...
    if response.code != 200:
      #raise newException(FidgetError, "Downloading " & url & " failed")
      echo "failed to download custom font"
    writeFile(fontPath, response.body)

proc downloadFonts(figmaFile: FigmaFile) =
  if not dirExists(rootPath / "fonts"):
    createDir(rootPath / "fonts")
  ...
  proc incl(style: TypeStyle) =
    if style.fontFamily == "" and style.fontPostScriptName == "":
      # broken nameless font
      return
  ...
  for kind, path in walkDir(rootPath / "fonts", relative = true):
    case kind:
    of pcFile:
    ...
    of pcDir:
      removeDir(rootPath / "fonts" / path)
    of pcLinkToFile, pcLinkToDir:
      removeFile(rootPath / "fonts" / path)
...
proc downloadFigmaFile(fileKey: string) =
  ...
  # Walk data/{fidget-id} dir and remove any unexpected entries

  for kind, path in walkDir(rootPath, relative = true):
    case kind:
    of pcFile:
      let (_, name, _) = splitFile(path)
      if name != fileKey:
        removeFile(rootPath / path)
    of pcDir:
