import bumpy, chroma, common, jsony, std/strutils, std/tables, pixie, vmath,
  std/options, std/unicode, std/sets
...
type
  ...
  Node* = ref object
    ...
    # Export
    exportSettings*: seq[ExportSettings]
...
proc postHook(v: var TypeStyle) =
  if v.fontPostScriptName == "":
    let weight = case v.fontWeight:
      of 100: "Thin"
      of 200: "ExtraLight"
      of 300: "Light"
      of 400: "Regular"
    ...
    v.fontPostScriptName = v.fontFamily & "-" & weight
    echo "Fixing fontPostScriptName: ", v.fontPostScriptName
...
proc enumHook(s: string, v: var LineType) =
  v = case s:
    of "NONE": NoneLineType
    of "UNORDERED": UnorderedLineType
    else: raise newException(FidgetError, "Invalid line type:" & s)
...
proc enumHook(s: string, v: var LayoutPositioning) =
  v = case s:
    of "AUTO": LayoutPositioningAuto
    of "ABSOLUTE": LayoutPositioningAbsolute
    else: raise newException(FidgetError, "Invalid layout mode:" & s)

proc enumHook(s: string, v: var AxisAlign) =
  v = case s:
    of "MIN": MinAxisAlign
    of "CENTER": CenterAxisAlign
    of "MAX": MaxAxisAlign
    of "SPACE_BETWEEN": SpaceBetweenAxisAlign
    of "BASELINE": BaselineAxisAlign
    else: raise newException(FidgetError, "Invalid axis align:" & s)
...
proc parseHook(s: string, i: var int, v: var Path) =
  ...
  try:
    ...
    echo "invalid path:", pathString
    writeFile("badpath.txt", pathString)
...
proc parseHook(s: string, i: var int, node: var Node) =
  ...
  node.lineTypes = f.lineTypes
  node.constraints = f.constraints
  node.layoutAlign = f.layoutAlign
  node.layoutGrids = f.layoutGrids
  node.layoutMode = f.layoutMode
  node.layoutPositioning = f.layoutPositioning
  node.primaryAxisAlignItems = f.primaryAxisAlignItems
  node.counterAxisAlignItems = f.counterAxisAlignItems
  node.itemReverseZIndex = f.itemReverseZIndex
  ...
  node.exportSettings = f.exportSettings
  node.transitionNodeID = f.transitionNodeID
...
proc findById*(figmaFile: FigmaFile, id: string): Node =
  ...
  return walk(figmaFile.document)

# proc masterComponent*(node: Node): Node =
#   if node.componentId in figmaFile.components:
#     let key = figmaFile.components[node.componentId]
#     echo key
